import React from 'react';
import { createBottomTabNavigator } from '@react-navigation/bottom-tabs';
import { useTheme } from 'react-native-paper';
import {
  DashboardScreen,
  MeterScreen,
  UsageScreen,
  ProfileScreen
} from '../screens';
import { BillsStackNavigator } from './BillsStackNavigator';
import { DisputeStackNavigator } from './DisputeStackNavigator';
import { SupportStackNavigator } from './SupportStackNavigator';
import { ChatStackNavigator } from './ChatStackNavigator';
import { MaterialCommunityIcons } from '@expo/vector-icons';

export type RootTabParamList = {
  Dashboard: undefined;
  Bills: undefined;
  Dispute: undefined;
  Meter: undefined;
  Usage: undefined;
  Support: undefined;
  AIHelp: undefined;
  Profile: undefined;
};

const Tab = createBottomTabNavigator<RootTabParamList>();

export const AppNavigator: React.FC = () => {
  const paperTheme = useTheme();

  return (
    <Tab.Navigator
        screenOptions={({ route }) => ({
          headerShown: false,
          tabBarActiveTintColor: paperTheme.colors.primary,
          tabBarInactiveTintColor: paperTheme.colors.outline,
          tabBarStyle: {
            backgroundColor: paperTheme.colors.surface,
            borderTopWidth: 0,
            paddingBottom: 6,
            height: 70
          },
            tabBarIcon: ({ color, size }) => {
              const iconMap: Record<keyof RootTabParamList, string> = {
                Dashboard: 'view-dashboard-outline',
                Bills: 'receipt-text-outline',
                Dispute: 'alert-octagon-outline',
                Meter: 'camera-metering-matrix',
                Usage: 'chart-line',
                Support: 'account-question-outline',
                AIHelp: 'robot',
                Profile: 'account-circle-outline'
              };
              const iconName = iconMap[route.name as keyof RootTabParamList];
              return <MaterialCommunityIcons name={iconName as any} size={size} color={color} />;
            }
        })}
      >
        <Tab.Screen name="Dashboard" component={DashboardScreen} />
        <Tab.Screen name="Bills" component={BillsStackNavigator} />
        <Tab.Screen name="Dispute" component={DisputeStackNavigator} />
        <Tab.Screen name="Meter" component={MeterScreen} />
        <Tab.Screen name="Usage" component={UsageScreen} />
        <Tab.Screen name="Support" component={SupportStackNavigator} />
        <Tab.Screen name="AIHelp" component={ChatStackNavigator} />
        <Tab.Screen name="Profile" component={ProfileScreen} />
      </Tab.Navigator>
  );
};

